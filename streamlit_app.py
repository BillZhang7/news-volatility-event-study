...
# Research design constants (inlined so Cloud deploy needs no src/ imports)
HYPOTHESIS_ID = "H-A3"
HYPOTHESIS_TEXT = (
...
if not all(p.exists() for p in required):
    st.error("Missing output files in `outputs/`. Run the pipeline locally or check the GitHub repo.")
    st.code(
...
if (OUTPUT / "vol_event_curve.png").exists():
    st.image(str(OUTPUT / "vol_event_curve.png"), caption="Average volatility path around events")
